...
try:
    from problems.local.state_local import StateLOCAL, BsStateLOCAL
    from utils.beamsearch import beam_search
except:
    from node_permutation.AM_Revisier.problems.local.state_local import StateLOCAL, BsStateLOCAL
    from node_permutation.AM_Revisier.utils.beamsearch import beam_search
...
class LOCAL(object):
    ...
    @staticmethod
    def make_bs_state(*args, **kwargs):
        return BsStateLOCAL.initialize(*args, **kwargs)

    @staticmethod
    def get_bs_costs(dataset, pi):
        # dataset: (batch, graph, 2)
        # pi: (batch, beam, graph)
        batch_size, graph_size, _ = dataset.size()
        beam_size = pi.size(1)
        assert pi.size(2) == graph_size

        # (batch, beam, graph, 2)
        d = dataset[:, None, :, :].expand(batch_size, beam_size, graph_size, 2).gather(
            index=pi[:, :, :, None].expand(batch_size, beam_size, graph_size, 2), dim=2
        )

        # (batch, beam)
        return (d[:, :, 0:-1] - d[:, :, 1:]).norm(p=2, dim=3).sum(2), None
