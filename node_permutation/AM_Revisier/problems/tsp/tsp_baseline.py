...
#         assert abs(total_cost - cost) <= 1e-5, "Cost is incorrect"
        return total_cost, tour[len(tour)-1], duration
...
def solve_concorde_log(executable, directory, name, loc, disable_cache=False, is_shpp=False,is_pctsp=False):
        ...
        if True:
            ...
            if is_shpp or is_pctsp:
                ...
                if is_pctsp:
                    dist[n-1,0]=1000000
                    dist[0,n-1]=1000000
                write_shpplib(problem_filename, dist, name=name)
            ...
            if is_shpp or is_pctsp:
                if tour[1]==n-1:
                    ...
                    tour = np.concatenate((tour[-1:],tour[1:-1]), axis=0)
                    tour = tour[:-1]
                ...
                return calc_shpp_length(loc, tour), tour, duration    
...
def write_shpplib(filename, loc, name="problem"):
...
def calc_shpp_length(loc, tour):
    sorted_locs = np.array(loc)[tour]
    return np.linalg.norm(sorted_locs[1:] - sorted_locs[:-1], axis=-1).sum()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--problem', default='tsp', help="Name of results directory")
...
#                 "Target dir already exists! Try running with -f option to overwrite."
            ...
            if method == "concorde":
                ...
                if opts.problem == "shpp":
                    is_shpp = True
                if opts.problem == "pctsp":
                    is_pctsp = True
                    
                def run_func(args):
                    return solve_concorde_log(executable, *args, disable_cache=opts.disable_cache, is_shpp = is_shpp,is_pctsp=is_pctsp)
                ...
                def run_func(args):
                    return solve_gurobi(*args, disable_cache=opts.disable_cache,
                                        timeout=1000 if method[6:] == "t" else None,
